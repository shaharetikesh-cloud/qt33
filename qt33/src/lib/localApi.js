...
function isMonthlyExpired(userRow) {
  ...
  const base = Date.parse(getActivationStartIso(userRow))
...
export async function localSignIn(credentials) {
  ...
  if (profile && isMonthlyExpired(profile)) {
    await supabase
      ...
      .eq('id', profile.id)
    await signOut(firebaseAuth)
    throw new Error('30-day access expired. Main Admin/Owner ne account पुन्हा active करava.')
  }
  if (profile?.id) {
    await supabase
      .from('profiles')
...
export async function localGetWorkspaceConfig() {
  const [legacyMasters, settings, divisions, feeders, batterySets, transformers] = await Promise.all([
  ...
  return { masters: mergedMasters, settings: settings[0]?.payload || {}, updatedAt: new Date().toISOString() }
}
