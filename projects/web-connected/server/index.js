...
const defaultAdminPassword =
  process.env.LOCAL_ADMIN_PASSWORD || crypto.randomBytes(12).toString('base64url')
...
function initializeDatabase() {
  ...
  if (!existingUser) {
    ...
    if (!process.env.LOCAL_ADMIN_PASSWORD) {
      console.warn(
        'LOCAL_ADMIN_PASSWORD not set. Generated one-time bootstrap password is in-memory only.',
      )
    }
    return
  }
