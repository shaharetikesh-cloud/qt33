...
This folder is the **project-wise arranged copy** of the canonical deployed web + connected Capacitor project.

- Project root id: `web-connected-canonical`
- Guard script: `scripts/ensure-project-root.mjs`
- Deploy/build commands must run from this folder only
- Expected working directory: `projects/web-connected`

## Final working root (hard-pin)

For all web + connected APK changes, treat this as the only active root:

- `f:/qt33.in/unified_msedcl_workspace/unified_msedcl_workspace/projects/web-connected`

Do not use:

- workspace legacy root for day-to-day commands
- `qt33` mirror
...
- Deploy prep: `npm run build` (then deploy `dist/` artifact from this folder only)

## Env safety

- Do not copy `.env` from other projects.
- Use only these in this project:
  - `.env.example`
  - `.env.project.example`
  - `.env.offline` (offline mode only)
- Create local `.env` inside this folder only.
