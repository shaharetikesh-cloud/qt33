...
export function AuthProvider({ children }) {
  ...
  const isProfileLoginAllowed = useCallback(async (nextProfile, { enforceSignOut } = {}) => {
    if (!nextProfile) {
      ...
      throw new Error('Profile load zala nahi. Admin approval kiwa profile sync pending asu shakto.')
    }

    if (!nextProfile.is_active || nextProfile.approval_status === 'pending') {
      if (enforceSignOut) {
        if (isLocalSqlMode) {
          await localSignOut()
        } else if (supabase) {
      ...
      throw new Error('Tumcha profile approve/active jhalyanantarach access milel.')
    }
  ...
  const syncSupabaseSessionState = useCallback(
    async (event, nextSession) => {
      ...
      const nextProfile = await loadSupabaseProfile(nextSession.user.id)
      await isProfileLoginAllowed(nextProfile, { enforceSignOut: true })
    },
    [isProfileLoginAllowed, loadSupabaseProfile],
  )
