...
import { idbListOutboxByStatus, idbUpdateOutboxStatus } from '../lib/indexedDb'
...
export default function AppShell() {
  ...
  const brandTitle = 'QT33'
  const brandSubtitle = 'Utility Operations Platform'
  ...
  useEffect(() => {
    let active = true
    void idbListOutboxByStatus('failed', 5)
      .then((failedItems) => {
        if (active) {
  ...
  async function refreshFailedSyncItems() {
    try {
      const failedItems = await idbListOutboxByStatus('failed', 5)
      setFailedSyncItems(failedItems)
    } catch {
      setFailedSyncItems([])
    }
  ...
  async function handleRetrySyncItem(queueId) {
    await idbUpdateOutboxStatus(queueId, 'pending', {
      retry_count: 0,
      next_retry_at: Date.now(),
      last_error: '',
    })
    setManualSyncNote('Retry queued for failed record.')
    await refreshFailedSyncItems()
        ...
        <div className="workspace-header-inner">
          ...
          <div className="workspace-header-right">
            ...
            <label className="header-substation-picker" htmlFor="workspace-substation">
              ...
              <span>{`P${syncState.pending} / F${syncState.failed} / C${syncState.conflicts}`}</span>
              ...
              {failedSyncItems.length ? (
                <span className="muted-copy">
                  Failed items: {failedSyncItems.length}. Retry available below.
                </span>
              ) : null}
            </label>
        ...
        <main className="content-shell workspace-content-shell">
          ...
          {failedSyncItems.length ? (
            <section className="content-card" style={{ marginTop: '1rem' }}>
              <h3>Sync failures</h3>
              <p className="muted-copy">Recent failed records with manual retry.</p>
              <div className="details-grid">
                {failedSyncItems.map((item) => (
                  <article key={item.queue_id} className="detail-card">
                    <h4>{item.entity_type || item.scope || 'record'}</h4>
                    <p className="muted-copy">{item.id}</p>
                    <p className="muted-copy">{item.last_error || 'Sync failed due to unknown error.'}</p>
                    <button
                      type="button"
                      className="ghost-light-button"
                      onClick={() => void handleRetrySyncItem(item.queue_id)}
                    >
                      Retry
                    </button>
                  </article>
          ...
          <section className="content-card" style={{ marginTop: '1rem' }}>
            ...
            <p className="muted-copy">
              ...
              <a href="./terms-and-conditions.html" target="_blank" rel="noreferrer">Terms</a> |{' '}
