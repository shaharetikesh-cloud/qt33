...
async function renderCanvas(element) {
  ...
  try {
    ...
    const estimatedPixels = Math.max(width * height, 1)
    const dynamicScale = Math.min(
      REPORT_RENDER_SCALE,
      Math.max(Math.sqrt(MAX_CANVAS_PIXELS / estimatedPixels), 1),
    )
    const rowAnchors = Array.from(
      ...
      .map((row) => {
        ...
        return Math.max(Math.floor((rowRect.top - cloneRect.top) * dynamicScale), 0)
      })
