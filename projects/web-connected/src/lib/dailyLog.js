...
function getInterruptionDisplayHourRange(interruption) {
  const startMinutes = parseTimeToMinutes(interruption?.from_time)
  const endMinutes = parseTimeToMinutes(interruption?.to_time)

  if (startMinutes === null || endMinutes === null || endMinutes < startMinutes) {
    return null
  }

  if (String(interruption?.event_type || '').toUpperCase() === 'LS') {
    // Reading at the starting hour belongs to that slot already, so LS starts from the next slot.
    const startHourIndex = Math.max(0, Math.min(24, Math.floor(startMinutes / 60) + 1))
    const endHourIndex = Math.max(0, Math.min(24, Math.ceil(endMinutes / 60)))
    return endHourIndex < startHourIndex
      ? null
      : { startHourIndex, endHourIndex, includeEndHour: true }
  }

  return {
    startHourIndex: timeToHourIndex(interruption?.from_time),
    endHourIndex: timeToHourIndex(interruption?.to_time, true),
...
function buildExplicitOverlayMap(interruptions) {
  ...
  interruptions.forEach((interruption) => {
    const hourRange = getInterruptionDisplayHourRange(interruption)

    if (!hourRange) {
      return
    }

    const { startHourIndex, endHourIndex, includeEndHour } = hourRange
    const upperBound = includeEndHour ? endHourIndex + 1 : endHourIndex

    if (startHourIndex < 0 || endHourIndex < 0 || upperBound <= startHourIndex) {
      return
    }

    ;(interruption.affectedFeederIds || [interruption.feeder_id]).forEach((feederId) => {
      for (let hourIndex = startHourIndex; hourIndex < upperBound; hourIndex += 1) {
